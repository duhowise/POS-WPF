...
namespace Magentix.Presentation.Services.Implementations.UserModule
{
    ...
    public class UserService : AbstractService, IUserService
    {
        ...
        public bool CanConfirmAdminPin(string pinValue)
        {
            return this.IsUserPermittedFor(this.GetUserByPinCode(pinValue), PermissionNames.CanConfirmAdminPin);
        }

        public bool IsUserPermittedFor(User user, string permissionName)
        {
            if (user == User.Nobody)
            {
                return false;
            ...
            Permission permission = user.UserRole.Permissions.SingleOrDefault<Permission>((Permission x) => x.Name == permissionName);
            if (permission == null)
            {
                return false;
            }
