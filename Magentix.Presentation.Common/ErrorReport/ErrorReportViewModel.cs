...
using Magentix.Infrastructure.ExceptionReporter;
using Magentix.Localization.Properties;
using Magentix.Presentation.Common.Commands;

namespace Magentix.Presentation.Common.ErrorReport
{
    class ErrorReportViewModel : ObservableObject
    {
        ...
        public void SubmitError()
        {
            ...
            string queryString = string.Format("from={0}&emaila={1}&file={2}",
                Uri.EscapeDataString("info@Magentixpos.com"),
                Uri.EscapeDataString("MagentixPOS V3 Error Report"),
                Uri.EscapeDataString(tempFile));

            var c = new WebClient();
            var result = c.UploadFile("http://reports.Magentixpos.com/file.php?" + queryString, "POST", tempFile);
